<template>
  <div>
    <div class="navs">
      <router-link class="nav current" to="/modifyPhoneByPhoneNo">验证原手机</router-link>
      <router-link class="nav" to="/modifyPhoneByIdCard">验证身份证</router-link>
    </div>
    <form v-show="!finishStepOne">
      <div class="box">
        <label for="phoneNoView">手机号码</label><span class="msg gray" v-show="!errObj.phoneNotValid">输入绑定手机号码</span><span class="msg red" v-show="errObj.phoneNotValid">请输入正确的手机号码</span>
        <div class="inputbox">
...
<script>
  ...
  export default {
    ...
    computed: {
      btnIsActive () {
        return this.phoneIsValid && this.smsCode && this.transPwd
      },
      phoneIsValid () {
    ...
    methods: {
      togglePhone () {
        this.currentNav = 'phone'
      },
      toggleIdCard () {
        this.currentNav = 'idcard'
      },
      formatPhone (val) {
